...
const OnboardingEdit = () => {
    ...
    useEffect(() => {
        const fetchOnboardingData = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/onboarding/${id}`);
                ...
                setMessage({ text: 'Error loading form data. Please try again.', type: 'error' });
                setLoading(false);
            }
        };
    ...
    const handleFormSubmit = async (e) => {
        ...
        showMessage('Updating onboarding form...', 'info');
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/onboarding/update/${id}`, {
                method: 'PUT',
                body: formDataToSend,
            });
            ...
            if (response.ok) {
                showMessage('Onboarding form updated successfully. Your changes have been saved.', 'success');
                setTimeout(() => {
                    // Optionally redirect or reset form
                }, 2000);
            } else {
                showMessage(result.message || 'Error updating form.', 'error');
            }
        } catch (error) {
    ...
    if (loading) {
        return (
            <Container maxWidth="md" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh' }}>
                <CircularProgress />
                <Typography variant="h6" sx={{ ml: 2 }}>Loading form data...</Typography>
            </Container>
        );
    }
                ...
                >
                    <Box sx={{ textAlign: 'center', mb: 5 }}>
                        ...
                        <Typography color="text.secondary">
                            Update your onboarding details. Complete all 5 steps to save your changes.
                        </Typography>
                    </Box>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 2 }}>
                                ...
                                {currentStep === totalSteps && (
                                    ...
                                    >
                                        {isSubmitting ? 'Updating...' : 'Update Onboarding Form'}
                                    </Button>
                                )}
                            </Box>
                        </Box>
